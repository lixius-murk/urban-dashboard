-- ============================================
-- SEED DATA for simplified schema
-- ============================================
...
INSERT INTO plant_instances (id, name, id_species, height, pot_size, state, active, last_watered, last_checked, temp_min, soil_moisture_min, light_min) VALUES
                                                                                                                                                            (1, 'Monstera #1', 1, 45.0, 25, 0, true, NOW() - INTERVAL '2 days', NOW() - INTERVAL '1 hour', 18.0, 30, 2000),
...
-- 5. Telemetry data - ONE record per plant with ALL values populated
INSERT INTO telemetry (id, id_plant, id_sensor, temp, humidity, soil_moisture, light, timestamp, source) VALUES
                                                                                                             -- Monstera #1 - using sensor 1 (TEMPERATURE) but with all values
                                                                                                             (1, 1, 1, 23.5, 55, 42, 2500, NOW() - INTERVAL '2 hours', 'SIMULATOR'),
                                                                                                             -- Fiddle Leaf Fig - using sensor 6 (TEMPERATURE) but with all values
                                                                                                             (2, 2, 6, 22.0, 48, 38, 3200, NOW() - INTERVAL '1 hour', 'SIMULATOR'),
                                                                                                             -- Snake Plant - using sensor 11 (TEMPERATURE) but with all values
                                                                                                             (3, 3, 11, 25.0, 35, 28, 1800, NOW() - INTERVAL '3 hours', 'SIMULATOR'),
                                                                                                             -- Peace Lily - using sensor 16 (TEMPERATURE) but with all values
...
INSERT INTO events (id, id_plant, type, action, time, status) VALUES
                                                                  ...
                                                                  (4, 4, 'WATERING', 'Low soil moisture detected', NOW() - INTERVAL '12 hours', 0);
